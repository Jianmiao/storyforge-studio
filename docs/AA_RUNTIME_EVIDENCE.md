...
AA 文件直接证实的是 **五个前景槽**，并非六个。HaloCue/StoryForge 的标准演出模型需要六槽，因此本项目采用稳定六槽 `1..6`，在 AA 的可见外缘 `-925..925` 内等距分布：`-925,-555,-185,185,555,925`。这是 StoryForge 的显式适配，不冒充 AA 原始常量。

兼容策略：旧项目仅有 `slot: 0/1/2` 时继续使用既有的 `0.26W/0.5W/0.74W`；只有出现 `startSlot` 或 `endSlot` 才启用六槽语义。默认移动为证据确认的 `0.5s + easeInOut`。未高亮角色采用证据确认的 `0.6` 亮度。
